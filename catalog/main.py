...
# Read Google client_secrets.json and assign client_id to a variable for
# later use.
...
def editCategory(category_name):
    ...
    if categoryToEdit.user_id != login_session['user_id']:
        scr = '<script>function myFunction()'
        scr += ''' {alert('You are not authorized to edit. '''
        scr += '''Please create your own'''
        scr += '''category in order to edit. '''
        scr += ''' ');}</script><body onload='myFunction()'>'''
        return scr
...
def deleteCategory(category_name):
    ...
    if categoryToDelete.user_id != login_session['user_id']:
        scr = '<script>function myFunction()'
        scr += ''' {alert('You are not authorized to delete. '''
        scr += '''Please create your own'''
        scr += '''category in order to delete.'''
        scr += ''' ');}</script><body onload='myFunction()'>'''
        return scr
...
@app.route(
    '/catalog/<category_name>/<item_name>/editimage',
    methods=[
        'GET',
        'POST'])
@login_required
def editImage(category_name, item_name):
    ...
    if editingItem.user_id != login_session['user_id']:
        ...
        scr += '''Please create your own'''
        scr += '''category in order to edit. '''
        scr += '''in order to edit.');}</script><body onload='myFunction()'>'''
        return scr
